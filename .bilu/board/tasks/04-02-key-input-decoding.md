...
Task: `src/board/tasks/04-02-key-input-decoding.md`

This implementation plan defines how the bash TUI reads and decodes keys reliably (arrows + fallbacks) without blocking redraws. It follows `src/storage/research/shell-only-cli-advanced-notes.md` and the `fff`-style approach: read one byte, and if it’s ESC, read a few more bytes to decode sequences.
...
- Implemented `tui_read_key` in `.bilu/cli/commands/board/render/tui.sh` (non-blocking read + escape-sequence decoding for arrows/Home/End/Page where available); verified basic TUI behavior via manual stub and updated tests pass.
# Priority
MEDIUM
# Kind
task
