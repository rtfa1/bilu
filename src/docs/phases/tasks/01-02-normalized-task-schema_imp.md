...
Task: `src/docs/phases/tasks/01-02-normalized-task-schema.md`
...
- `title` (string)
- `status` (enum)
  - canonical set from `src/board/config.json.statuses`
- `priority` (enum)
  - canonical set from `src/board/config.json.priorities`
- `path` (string)
  - filesystem path to the task markdown file (absolute or repo-relative, choose one).

### Optional fields

- `description` (string)
- `kind` (enum)
  - canonical set from `src/board/config.json.kind`
...
- `id` = filename without extension, e.g.
  - `src/board/tasks/01-project-scaffold-and-manifest.md`
...
Per `src/docs/phases/tasks/02-06-internal-record-format.md`, define a single strict TSV with stable column order.
...
- A single TSV format is documented and referenced by:
  - `src/docs/phases/tasks/02-06-internal-record-format.md`
...
- `src/docs/phases/tasks/01-02-normalized-task-schema.md`
- `src/docs/phases/tasks/01-03-normalization-rules.md`
- `src/docs/phases/tasks/02-06-internal-record-format.md`
