...
Task: `src/docs/phases/tasks/06-02-cli-parsing-tests.md`

This plan locks down the `bilu board` option parser with portable, deterministic shell tests. It follows `src/docs/research/shell-only-cli-advanced-notes.md`: explicit flag parsing behavior, predictable exit code `2` for usage errors, and command-oriented tests (assert exit code + stdout/stderr patterns).
...
From the parser spec (`src/docs/phases/tasks/02-04-args-parser_imp.md`):
...
- Parser behavior matches `02-04-args-parser_imp.md` exactly.
- Unknown flags and missing values consistently return exit code `2`.

## References

- `src/docs/phases/tasks/06-02-cli-parsing-tests.md`
- `src/docs/phases/tasks/02-04-args-parser_imp.md`
