...
Task: `src/board/tasks/05-05-file-locking-and-concurrency.md`

This plan defines a simple, portable lock strategy to prevent concurrent commands from corrupting files. It follows the guidance in `src/storage/research/shell-only-cli-advanced-notes.md`: use mkdir-based locks, always release via `trap`, and keep writes atomic (temp file + `mv`).
...
- `src/board/tasks/05-04-rebuild-index-command_imp.md`
